...
def rescale_microstructure(spin_array, dims, origin=[0, 0, 0]):
    ...
    new_spin_array = spin_array.copy()
    print(iter_x, iter_y, iter_z)
    ...
    intermediate_z = new_spin_array.copy()

    for i in range(iter_z):
        extra_zrows = intermediate_z[:, :, :]
